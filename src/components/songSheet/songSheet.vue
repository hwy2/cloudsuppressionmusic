<template>
  <div class="dialog-mask songSheet">
        ...
        <div class="containerDiaLog"
          ...
          <div class="topBanner songListTopBanner songSheetTopBanner">
            ...
            <div class="navbarSorll"
              ...
              <ul class="clearfix"
                  id="navbarSorllul">
                <li :class="{active:item.activity}"
                    id="item.id"
                    v-for="(item,index) in categoryHot"
                    :key="item.id"
                    @click="gethighquality(index,item.name)">{{item.name}}</li>
              </ul>
              <div class="more">
                <i class="iconfont icongengduo3"
                   @click="openPopup()"></i>
              </div>
            </div>
          </div>
          ...
          <!-- song LIST -->
          <div class="sheet">
            <mt-loadmore :bottomMethod="loadBottom"
                         :bottom-all-loaded="allLoaded"
                         :auto-fill="false"
                         ref="loadmore"
                         id="loadmore">
              <ul class="clearfix">
                <li v-for="(item,i) in highqualityList"
                    :key="i"
...
export default {
    ...
    subSelect: function (name) {
      ...
      if (!checkTag) {
        ...
        if (checkTag > selectedTag) {
          this.$refs.navbarSorllall.scrollLeft = (checkTag - selectedTag) * step;
        } else {
          this.$refs.navbarSorllall.scrollLeft = -(selectedTag - checkTag) * step;
        }
      }
  ...
  watch: {
    popupSongVisible: function (newV) {
      //   window.console.log("当前分类弹出层：", newV);
      if (newV) {
